...
attributeTextNodes
  = '"' a:attributeTextNodesInner closingDoubleQuote { return a; }
  / "'" a:attributeTextNodesInnerSingle closingSingleQuote { return a; }
...
attributeTextNodesInner
  = first:preAttrMustacheText? tail:(mustacheStatement preAttrMustacheText?)*
{
  return flattenArray(first, tail);
}

attributeTextNodesInnerSingle
  = first:preAttrMustacheTextSingle? tail:(mustacheStatement preAttrMustacheTextSingle?)*
{
  return flattenArray(first, tail);
}
...
preAttrMustacheText
  = $preAttrMustacheUnit+
preAttrMustacheTextSingle
  = $preAttrMustacheUnitSingle+

preAttrMustacheUnit 'Valid quoted attribute value'
  = !(nonMustacheUnit / '"') c:. { return c; }
preAttrMustacheUnitSingle 'Valid quoted attribute value'
  = !(nonMustacheUnit / "'") c:. { return c; }
