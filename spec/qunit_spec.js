...
Handlebars.registerHelper('view', function(param) {
  var content, options;
  options = arguments[arguments.length - 1];
  content = param;
  if (options.fn) {
...
test("should invoke `view` helper by default", function() {
  var emblem;
  emblem = "SomeView";
  return shouldCompileTo(emblem, '<view class="SomeView">SomeView</view>');
});
...
Handlebars.registerHelper('unbound', function() {
  ...
  return new Handlebars.SafeString("<view class=\"" + stringedParams + "\">" + content + "</view>'");
});

test("bang helper defaults to `unbound` invocation", function() {
  var emblem;
  emblem = "foo! Yar\n= foo!";
  return shouldCompileTo(emblem, '<unbound class="foo Yar">foo Yar</unbound><unbound class="foo">foo</unbound>');
});

test("bang helper works with blocks", function() {
  var emblem;
  emblem = "hey! you suck\n  = foo!";
  return shouldCompileTo(emblem, '<unbound class="hey you suck"><unbound class="foo">foo</unbound></unbound>');
});
...
test("basic", function() {
  return shouldCompileTo('p class=foo', '<p classBoundTofoo></p>');
});

test("multiple", function() {
  return shouldCompileTo('p class=foo id="yup" data-thinger=yeah Hooray', '<p classBoundTofoo id="yup" data-thingBoundToyeah>Hooray</p>');
});

test("class bindAttr special syntax", function() {
  return shouldCompileTo('p class=foo:bar:baz', '<p classBoundTofoo:bar:baz></p>');
});

suite("in-tag explicit mustache");

test("basic", function() {
  return shouldCompileTo('p{{bindAttr class="foo"}}', '<p classBoundTofoo></p>');
});

test("with inline content", function() {
  var emblem;
  emblem = "p{{bindAttr class=\"foo\"}} Hello\np{{bindAttr class=\"foo\"}} = foo";
  return shouldCompileTo(emblem, {
    foo: "yar"
...
test("multiple", function() {
  var emblem;
  emblem = "p{{bindAttr class=\"foo\"}}{{bindAttr thing=\"yar\"}} Hello";
  return shouldCompileTo(emblem, {
    foo: "yar"
...
Handlebars.registerHelper('bindAttr', function(actionName) {
  var onString, options, params, targetString;
  options = arguments[arguments.length - 1];
  params = Array.prototype.slice.call(arguments, -1);
  onString = options.hash.on;
  targetString = "target=" + (options.hash.target || "default");
  return "action|" + onString + "|" + params[0] + "|" + targetString;
});

test("basic (click)", function() {
  var emblem;
  emblem = "button click=\"submitComment\" Submit Comment";
  return shouldCompileTo(emblem, '<button "action|click||target=default">Submit Comment</button>');
});

test("nested (mouseEnter)", function() {
  var emblem;
  emblem = "a mouseEnter='submitComment target=\"view\"'\n  | Submit Comment";
  return shouldCompileTo(emblem, '<a "action|mouseEnter||target=view">Submit Comment</a>');
});
