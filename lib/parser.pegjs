...
fullAttribute
  = ' '+ a:(stringActionAttribute / mustacheActionAttribute / booleanAttribute / boundAttributeWithSingleMustache/ boundAttribute / rawMustacheAttribute / normalAttribute)
{
  return a || [];
}

bracketedAttribute
= INDENT* ' '* a:(stringActionAttribute / mustacheActionAttribute / booleanAttribute / boundAttribute / rawMustacheAttribute / normalAttribute) TERM*
{
  if (a.length) {
    return a;
  } else {
...
mustacheActionAttribute
  = event:knownEvent '=' singleOpen _ mustacheNode:allCharactersExceptColonSyntax _ singleClose
{
  var actionContent = [];
