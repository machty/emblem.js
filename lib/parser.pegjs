...
lineStartingMustache
  = content:(capitalizedLineStarterMustache / mustacheOrBlock)
{
  builder.mustache(content, false);
}
...
mustacheOrBlock = mustacheContent:inMustache _ inlineComment?nestedContentProgramNode:mustacheNestedContent
{
...
explicitMustache = e:equalSign mustacheContent:mustacheOrBlock
{
  return builder.generateMustache(mustacheContent, e);
}
...
textLine = s:textLineStart nodes:textNodes indentedNodes:(indentation whitespaceableTextNodes* DEDENT)?
{
  ...
  builder.addChildNodes(castStringsToTextNodes(retWithNewlines));
}
...
{
  return builder.generateMustache(content, true);
}
...
{
  return builder.generateMustache(content, false);
}
