module.exports = (function(){
  /*
   * Generated by PEG.js 0.7.0.
   *
   * http://pegjs.majda.cz/
   */
  
  function subclass(child, parent) {
    function ctor() { this.constructor = child; }
    ctor.prototype = parent.prototype;
    child.prototype = new ctor();
  }
  
  function quote(s) {
    /*
     * ECMA-262, 5th ed., 7.8.4: All characters may appear literally in a
     * string literal except for the closing quote character, backslash,
     * carriage return, line separator, paragraph separator, and line feed.
     * Any character may appear in the form of an escape sequence.
     *
     * For portability, we also escape escape all control and non-ASCII
     * characters. Note that "\0" and "\v" escape sequences are not used
     * because JSHint does not like the first and IE the second.
     */
     return '"' + s
      .replace(/\\/g, '\\\\')  // backslash
      .replace(/"/g, '\\"')    // closing quote character
      .replace(/\x08/g, '\\b') // backspace
      .replace(/\t/g, '\\t')   // horizontal tab
      .replace(/\n/g, '\\n')   // line feed
      .replace(/\f/g, '\\f')   // form feed
      .replace(/\r/g, '\\r')   // carriage return
      .replace(/[\x00-\x07\x0B\x0E-\x1F\x80-\uFFFF]/g, escape)
      + '"';
  }
  
  var result = {
    /*
     * Parses the input with a generated parser. If the parsing is successful,
     * returns a value explicitly or implicitly specified by the grammar from
     * which the parser was generated (see |PEG.buildParser|). If the parsing is
     * unsuccessful, throws |PEG.parser.SyntaxError| describing the error.
     */
    parse: function(input) {
      var parseFunctions = {
        "start": parse_start
      };
      
      var options = arguments.length > 1 ? arguments[1] : {},
          startRule;
      
      if (options.startRule !== undefined) {
        startRule = options.startRule;
        
        if (parseFunctions[startRule] === undefined) {
          throw new Error("Can't start parsing from rule " + quote(startRule) + ".");
        }
      } else {
        startRule = "start";
      }
      
      var pos = 0;
      var reportedPos = 0;
      var cachedReportedPos = 0;
      var cachedReportedPosDetails = { line: 1, column: 1, seenCR: false };
      var reportFailures = 0;
      var rightmostFailuresPos = 0;
      var rightmostFailuresExpected = [];
      var cache = {};
      
      function padLeft(input, padding, length) {
        var result = input;
        
        var padLength = length - input.length;
        for (var i = 0; i < padLength; i++) {
          result = padding + result;
        }
        
        return result;
      }
      
      function escape(ch) {
        var charCode = ch.charCodeAt(0);
        var escapeChar;
        var length;
        
        if (charCode <= 0xFF) {
          escapeChar = 'x';
          length = 2;
        } else {
          escapeChar = 'u';
          length = 4;
        }
        
        return '\\' + escapeChar + padLeft(charCode.toString(16).toUpperCase(), '0', length);
      }
      
      function computeReportedPosDetails() {
        function advanceCachedReportedPos() {
          var ch;
          
          for (; cachedReportedPos < reportedPos; cachedReportedPos++) {
            ch = input.charAt(cachedReportedPos);
            if (ch === "\n") {
              if (!cachedReportedPosDetails.seenCR) { cachedReportedPosDetails.line++; }
              cachedReportedPosDetails.column = 1;
              cachedReportedPosDetails.seenCR = false;
            } else if (ch === "\r" || ch === "\u2028" || ch === "\u2029") {
              cachedReportedPosDetails.line++;
              cachedReportedPosDetails.column = 1;
              cachedReportedPosDetails.seenCR = true;
            } else {
              cachedReportedPosDetails.column++;
              cachedReportedPosDetails.seenCR = false;
            }
          }
        }
        
        if (cachedReportedPos !== reportedPos) {
          if (cachedReportedPos > reportedPos) {
            cachedReportedPos = 0;
            cachedReportedPosDetails = { line: 1, column: 1, seenCR: false };
          }
          advanceCachedReportedPos();
        }
        
        return cachedReportedPosDetails;
      }
      
      function text() {
        return input.substring(reportedPos, pos);
      }
      
      function offset() {
        return reportedPos;
      }
      
      function line() {
        return computeReportedPosDetails().line;
      }
      
      function column() {
        return computeReportedPosDetails().column;
      }
      
      function matchFailed(failure) {
        if (pos < rightmostFailuresPos) {
          return;
        }
        
        if (pos > rightmostFailuresPos) {
          rightmostFailuresPos = pos;
          rightmostFailuresExpected = [];
        }
        
        rightmostFailuresExpected.push(failure);
      }
      
      function parse_start() {
        var cacheKey = "start@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2, r3, r4, r5;
        
        r1 = pos;
        r2 = pos;
        r3 = [];
        r4 = parse_sourceLine();
        while (r4 !== null) {
          r3.push(r4);
          r4 = parse_sourceLine();
        }
        if (r3 !== null) {
          r4 = [];
          r5 = parse_ignorableLine();
          while (r5 !== null) {
            r4.push(r5);
            r5 = parse_ignorableLine();
          }
          if (r4 !== null) {
            r0 = [r3, r4];
          } else {
            r0 = null;
            pos = r2;
          }
        } else {
          r0 = null;
          pos = r2;
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function() { return sourceLine; })();
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_sourceLine() {
        var cacheKey = "sourceLine@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2, r3, r4, r5;
        
        r1 = pos;
        r2 = pos;
        r3 = [];
        r4 = parse_ignorableLine();
        while (r4 !== null) {
          r3.push(r4);
          r4 = parse_ignorableLine();
        }
        if (r3 !== null) {
          r4 = parse_sourceContent();
          if (r4 !== null) {
            r5 = parse_newline();
            r5 = r5 !== null ? r5 : "";
            if (r5 !== null) {
              r0 = [r3, r4, r5];
            } else {
              r0 = null;
              pos = r2;
            }
          } else {
            r0 = null;
            pos = r2;
          }
        } else {
          r0 = null;
          pos = r2;
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(s) { return s; })(r4);
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_sourceContent() {
        var cacheKey = "sourceContent@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1;
        
        r1 = pos;
        r0 = parse_htmlElement();
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(el) { return el;})(r0);
        }
        if (r0 === null) {
          pos = r1;
        }
        if (r0 === null) {
          r0 = parse_text();
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_hbInvocation() {
        var cacheKey = "hbInvocation@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1;
        
        r1 = pos;
        r0 = parse_text();
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(t) { return t; })(r0);
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_htmlElement() {
        var cacheKey = "htmlElement@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1;
        
        r1 = pos;
        r0 = parse_htmlElementOneLiner();
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(el) {return el;})(r0);
        }
        if (r0 === null) {
          pos = r1;
        }
        if (r0 === null) {
          r1 = pos;
          r0 = parse_htmlElementOpenTag();
          if (r0 !== null) {
            reportedPos = r1;
            r0 = (function(el) {return el;})(r0);
          }
          if (r0 === null) {
            pos = r1;
          }
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_htmlElementOneLiner() {
        var cacheKey = "htmlElementOneLiner@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2, r3, r4;
        
        r1 = pos;
        r2 = pos;
        r3 = parse_htmlElementOpenTag();
        if (r3 !== null) {
          r4 = parse_text();
          if (r4 !== null) {
            r0 = [r3, r4];
          } else {
            r0 = null;
            pos = r2;
          }
        } else {
          r0 = null;
          pos = r2;
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(el, t) { 
          el.oneLiner = true; 
          return el;
        })(r3, r4);
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_htmlElementOpenTag() {
        var cacheKey = "htmlElementOpenTag@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2, r3, r4;
        
        r1 = pos;
        r2 = pos;
        r3 = parse_htmlTagName();
        if (r3 !== null) {
          r4 = parse_tagShortcuts();
          if (r4 !== null) {
            r0 = [r3, r4];
          } else {
            r0 = null;
            pos = r2;
          }
        } else {
          r0 = null;
          pos = r2;
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(h, t) { return { tagName: h,    attrs: t  }; })(r3, r4);
        }
        if (r0 === null) {
          pos = r1;
        }
        if (r0 === null) {
          r1 = pos;
          r0 = parse_htmlTagName();
          if (r0 !== null) {
            reportedPos = r1;
            r0 = (function(h) { return { tagName: h,    attrs: {} }; })(r0);
          }
          if (r0 === null) {
            pos = r1;
          }
          if (r0 === null) {
            r1 = pos;
            r0 = parse_tagShortcuts();
            if (r0 !== null) {
              reportedPos = r1;
              r0 = (function(t) { return { tagName: null, attrs: t  }; })(r0);
            }
            if (r0 === null) {
              pos = r1;
            }
          }
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_tagShortcuts() {
        var cacheKey = "tagShortcuts@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2, r3, r4, r5;
        
        r1 = pos;
        r2 = pos;
        r3 = parse_idShortcut();
        r3 = r3 !== null ? r3 : "";
        if (r3 !== null) {
          r4 = [];
          r5 = parse_classShortcut();
          while (r5 !== null) {
            r4.push(r5);
            r5 = parse_classShortcut();
          }
          if (r4 !== null) {
            r0 = [r3, r4];
          } else {
            r0 = null;
            pos = r2;
          }
        } else {
          r0 = null;
          pos = r2;
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(id, classes) {
        
          var attrs = {};
          if(id) { attrs.id = id; }
          if(classes.length) { attrs['class'] = classes; }
        
          return attrs;
        })(r3, r4);
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_classShortcut() {
        var cacheKey = "classShortcut@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2, r3, r4;
        
        r1 = pos;
        r2 = pos;
        if (input.charCodeAt(pos) === 46) {
          r3 = ".";
          pos++;
        } else {
          r3 = null;
          if (reportFailures === 0) {
            matchFailed("\".\"");
          }
        }
        if (r3 !== null) {
          r4 = parse_ident();
          if (r4 !== null) {
            r0 = [r3, r4];
          } else {
            r0 = null;
            pos = r2;
          }
        } else {
          r0 = null;
          pos = r2;
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(c) { return c; })(r4);
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_ident() {
        var cacheKey = "ident@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2, r3, r4, r5;
        
        r1 = pos;
        r2 = pos;
        r3 = parse_nmstart();
        if (r3 !== null) {
          r4 = [];
          r5 = parse_nmchar();
          while (r5 !== null) {
            r4.push(r5);
            r5 = parse_nmchar();
          }
          if (r4 !== null) {
            r0 = [r3, r4];
          } else {
            r0 = null;
            pos = r2;
          }
        } else {
          r0 = null;
          pos = r2;
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(nmstart, nmchars) {
              return nmstart + nmchars.join("");
            })(r3, r4);
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_nmchar() {
        var cacheKey = "nmchar@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0;
        
        if (/^[_a-zA-Z0-9\-]/.test(input.charAt(pos))) {
          r0 = input.charAt(pos);
          pos++;
        } else {
          r0 = null;
          if (reportFailures === 0) {
            matchFailed("[_a-zA-Z0-9\\-]");
          }
        }
        if (r0 === null) {
          r0 = parse_nonascii();
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_nonascii() {
        var cacheKey = "nonascii@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0;
        
        if (/^[\x80-\xFF]/.test(input.charAt(pos))) {
          r0 = input.charAt(pos);
          pos++;
        } else {
          r0 = null;
          if (reportFailures === 0) {
            matchFailed("[\\x80-\\xFF]");
          }
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_nmstart() {
        var cacheKey = "nmstart@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0;
        
        if (/^[_a-zA-Z]/.test(input.charAt(pos))) {
          r0 = input.charAt(pos);
          pos++;
        } else {
          r0 = null;
          if (reportFailures === 0) {
            matchFailed("[_a-zA-Z]");
          }
        }
        if (r0 === null) {
          r0 = parse_nonascii();
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_idShortcut() {
        var cacheKey = "idShortcut@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2, r3, r4;
        
        r1 = pos;
        r2 = pos;
        if (input.charCodeAt(pos) === 35) {
          r3 = "#";
          pos++;
        } else {
          r3 = null;
          if (reportFailures === 0) {
            matchFailed("\"#\"");
          }
        }
        if (r3 !== null) {
          r4 = parse_ident();
          if (r4 !== null) {
            r0 = [r3, r4];
          } else {
            r0 = null;
            pos = r2;
          }
        } else {
          r0 = null;
          pos = r2;
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(t) { return t;})(r4);
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_ignorableLine() {
        var cacheKey = "ignorableLine@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0;
        
        r0 = parse_emptyLine();
        if (r0 === null) {
          r0 = parse_commentLine();
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_emptyLine() {
        var cacheKey = "emptyLine@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2, r3;
        
        r1 = pos;
        r2 = [];
        r3 = parse_indentSpace();
        while (r3 !== null) {
          r2.push(r3);
          r3 = parse_indentSpace();
        }
        if (r2 !== null) {
          r3 = parse_newline();
          if (r3 !== null) {
            r0 = [r2, r3];
          } else {
            r0 = null;
            pos = r1;
          }
        } else {
          r0 = null;
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_commentLine() {
        var cacheKey = "commentLine@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2, r3, r4, r5;
        
        r1 = pos;
        r2 = [];
        r3 = parse_indentSpace();
        while (r3 !== null) {
          r2.push(r3);
          r3 = parse_indentSpace();
        }
        if (r2 !== null) {
          if (input.charCodeAt(pos) === 47) {
            r3 = "/";
            pos++;
          } else {
            r3 = null;
            if (reportFailures === 0) {
              matchFailed("\"/\"");
            }
          }
          if (r3 !== null) {
            r4 = parse_text();
            if (r4 !== null) {
              r5 = parse_newline();
              if (r5 !== null) {
                r0 = [r2, r3, r4, r5];
              } else {
                r0 = null;
                pos = r1;
              }
            } else {
              r0 = null;
              pos = r1;
            }
          } else {
            r0 = null;
            pos = r1;
          }
        } else {
          r0 = null;
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_indentSpace() {
        var cacheKey = "indentSpace@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0;
        
        if (/^[ \t]/.test(input.charAt(pos))) {
          r0 = input.charAt(pos);
          pos++;
        } else {
          r0 = null;
          if (reportFailures === 0) {
            matchFailed("[ \\t]");
          }
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_indent() {
        var cacheKey = "indent@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2;
        
        r1 = pos;
        r0 = [];
        if (input.charCodeAt(pos) === 32) {
          r2 = " ";
          pos++;
        } else {
          r2 = null;
          if (reportFailures === 0) {
            matchFailed("\" \"");
          }
        }
        while (r2 !== null) {
          r0.push(r2);
          if (input.charCodeAt(pos) === 32) {
            r2 = " ";
            pos++;
          } else {
            r2 = null;
            if (reportFailures === 0) {
              matchFailed("\" \"");
            }
          }
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(s) {
          return indent(s) 
        })(r0);
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_text() {
        var cacheKey = "text@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2;
        
        r1 = pos;
        r0 = [];
        if (/^[^\n]/.test(input.charAt(pos))) {
          r2 = input.charAt(pos);
          pos++;
        } else {
          r2 = null;
          if (reportFailures === 0) {
            matchFailed("[^\\n]");
          }
        }
        while (r2 !== null) {
          r0.push(r2);
          if (/^[^\n]/.test(input.charAt(pos))) {
            r2 = input.charAt(pos);
            pos++;
          } else {
            r2 = null;
            if (reportFailures === 0) {
              matchFailed("[^\\n]");
            }
          }
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(c) { 
          return c.join("")
        })(r0);
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_newline() {
        var cacheKey = "newline@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1;
        
        r1 = pos;
        if (input.charCodeAt(pos) === 10) {
          r0 = "\n";
          pos++;
        } else {
          r0 = null;
          if (reportFailures === 0) {
            matchFailed("\"\\n\"");
          }
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function() {})();
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      function parse_htmlTagName() {
        var cacheKey = "htmlTagName@" + pos;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = cachedResult.nextPos;
          return cachedResult.result;
        }
        
        var r0, r1, r2, r3, r4, r5, r6;
        
        r1 = pos;
        r2 = pos;
        if (input.substr(pos, 10) === "figcaption") {
          r3 = "figcaption";
          pos += 10;
        } else {
          r3 = null;
          if (reportFailures === 0) {
            matchFailed("\"figcaption\"");
          }
        }
        if (r3 === null) {
          if (input.substr(pos, 10) === "blockquote") {
            r3 = "blockquote";
            pos += 10;
          } else {
            r3 = null;
            if (reportFailures === 0) {
              matchFailed("\"blockquote\"");
            }
          }
          if (r3 === null) {
            if (input.substr(pos, 9) === "plaintext") {
              r3 = "plaintext";
              pos += 9;
            } else {
              r3 = null;
              if (reportFailures === 0) {
                matchFailed("\"plaintext\"");
              }
            }
            if (r3 === null) {
              if (input.substr(pos, 8) === "textarea") {
                r3 = "textarea";
                pos += 8;
              } else {
                r3 = null;
                if (reportFailures === 0) {
                  matchFailed("\"textarea\"");
                }
              }
              if (r3 === null) {
                if (input.substr(pos, 8) === "progress") {
                  r3 = "progress";
                  pos += 8;
                } else {
                  r3 = null;
                  if (reportFailures === 0) {
                    matchFailed("\"progress\"");
                  }
                }
                if (r3 === null) {
                  if (input.substr(pos, 8) === "optgroup") {
                    r3 = "optgroup";
                    pos += 8;
                  } else {
                    r3 = null;
                    if (reportFailures === 0) {
                      matchFailed("\"optgroup\"");
                    }
                  }
                  if (r3 === null) {
                    if (input.substr(pos, 8) === "noscript") {
                      r3 = "noscript";
                      pos += 8;
                    } else {
                      r3 = null;
                      if (reportFailures === 0) {
                        matchFailed("\"noscript\"");
                      }
                    }
                    if (r3 === null) {
                      if (input.substr(pos, 8) === "noframes") {
                        r3 = "noframes";
                        pos += 8;
                      } else {
                        r3 = null;
                        if (reportFailures === 0) {
                          matchFailed("\"noframes\"");
                        }
                      }
                      if (r3 === null) {
                        if (input.substr(pos, 8) === "frameset") {
                          r3 = "frameset";
                          pos += 8;
                        } else {
                          r3 = null;
                          if (reportFailures === 0) {
                            matchFailed("\"frameset\"");
                          }
                        }
                        if (r3 === null) {
                          if (input.substr(pos, 8) === "fieldset") {
                            r3 = "fieldset";
                            pos += 8;
                          } else {
                            r3 = null;
                            if (reportFailures === 0) {
                              matchFailed("\"fieldset\"");
                            }
                          }
                          if (r3 === null) {
                            if (input.substr(pos, 8) === "datalist") {
                              r3 = "datalist";
                              pos += 8;
                            } else {
                              r3 = null;
                              if (reportFailures === 0) {
                                matchFailed("\"datalist\"");
                              }
                            }
                            if (r3 === null) {
                              if (input.substr(pos, 8) === "colgroup") {
                                r3 = "colgroup";
                                pos += 8;
                              } else {
                                r3 = null;
                                if (reportFailures === 0) {
                                  matchFailed("\"colgroup\"");
                                }
                              }
                              if (r3 === null) {
                                if (input.substr(pos, 8) === "basefont") {
                                  r3 = "basefont";
                                  pos += 8;
                                } else {
                                  r3 = null;
                                  if (reportFailures === 0) {
                                    matchFailed("\"basefont\"");
                                  }
                                }
                                if (r3 === null) {
                                  if (input.substr(pos, 7) === "summary") {
                                    r3 = "summary";
                                    pos += 7;
                                  } else {
                                    r3 = null;
                                    if (reportFailures === 0) {
                                      matchFailed("\"summary\"");
                                    }
                                  }
                                  if (r3 === null) {
                                    if (input.substr(pos, 7) === "section") {
                                      r3 = "section";
                                      pos += 7;
                                    } else {
                                      r3 = null;
                                      if (reportFailures === 0) {
                                        matchFailed("\"section\"");
                                      }
                                    }
                                    if (r3 === null) {
                                      if (input.substr(pos, 7) === "marquee") {
                                        r3 = "marquee";
                                        pos += 7;
                                      } else {
                                        r3 = null;
                                        if (reportFailures === 0) {
                                          matchFailed("\"marquee\"");
                                        }
                                      }
                                      if (r3 === null) {
                                        if (input.substr(pos, 7) === "listing") {
                                          r3 = "listing";
                                          pos += 7;
                                        } else {
                                          r3 = null;
                                          if (reportFailures === 0) {
                                            matchFailed("\"listing\"");
                                          }
                                        }
                                        if (r3 === null) {
                                          if (input.substr(pos, 7) === "isindex") {
                                            r3 = "isindex";
                                            pos += 7;
                                          } else {
                                            r3 = null;
                                            if (reportFailures === 0) {
                                              matchFailed("\"isindex\"");
                                            }
                                          }
                                          if (r3 === null) {
                                            if (input.substr(pos, 7) === "details") {
                                              r3 = "details";
                                              pos += 7;
                                            } else {
                                              r3 = null;
                                              if (reportFailures === 0) {
                                                matchFailed("\"details\"");
                                              }
                                            }
                                            if (r3 === null) {
                                              if (input.substr(pos, 7) === "command") {
                                                r3 = "command";
                                                pos += 7;
                                              } else {
                                                r3 = null;
                                                if (reportFailures === 0) {
                                                  matchFailed("\"command\"");
                                                }
                                              }
                                              if (r3 === null) {
                                                if (input.substr(pos, 7) === "caption") {
                                                  r3 = "caption";
                                                  pos += 7;
                                                } else {
                                                  r3 = null;
                                                  if (reportFailures === 0) {
                                                    matchFailed("\"caption\"");
                                                  }
                                                }
                                                if (r3 === null) {
                                                  if (input.substr(pos, 7) === "bgsound") {
                                                    r3 = "bgsound";
                                                    pos += 7;
                                                  } else {
                                                    r3 = null;
                                                    if (reportFailures === 0) {
                                                      matchFailed("\"bgsound\"");
                                                    }
                                                  }
                                                  if (r3 === null) {
                                                    if (input.substr(pos, 7) === "article") {
                                                      r3 = "article";
                                                      pos += 7;
                                                    } else {
                                                      r3 = null;
                                                      if (reportFailures === 0) {
                                                        matchFailed("\"article\"");
                                                      }
                                                    }
                                                    if (r3 === null) {
                                                      if (input.substr(pos, 7) === "address") {
                                                        r3 = "address";
                                                        pos += 7;
                                                      } else {
                                                        r3 = null;
                                                        if (reportFailures === 0) {
                                                          matchFailed("\"address\"");
                                                        }
                                                      }
                                                      if (r3 === null) {
                                                        if (input.substr(pos, 7) === "acronym") {
                                                          r3 = "acronym";
                                                          pos += 7;
                                                        } else {
                                                          r3 = null;
                                                          if (reportFailures === 0) {
                                                            matchFailed("\"acronym\"");
                                                          }
                                                        }
                                                        if (r3 === null) {
                                                          if (input.substr(pos, 6) === "strong") {
                                                            r3 = "strong";
                                                            pos += 6;
                                                          } else {
                                                            r3 = null;
                                                            if (reportFailures === 0) {
                                                              matchFailed("\"strong\"");
                                                            }
                                                          }
                                                          if (r3 === null) {
                                                            if (input.substr(pos, 6) === "strike") {
                                                              r3 = "strike";
                                                              pos += 6;
                                                            } else {
                                                              r3 = null;
                                                              if (reportFailures === 0) {
                                                                matchFailed("\"strike\"");
                                                              }
                                                            }
                                                            if (r3 === null) {
                                                              if (input.substr(pos, 6) === "spacer") {
                                                                r3 = "spacer";
                                                                pos += 6;
                                                              } else {
                                                                r3 = null;
                                                                if (reportFailures === 0) {
                                                                  matchFailed("\"spacer\"");
                                                                }
                                                              }
                                                              if (r3 === null) {
                                                                if (input.substr(pos, 6) === "source") {
                                                                  r3 = "source";
                                                                  pos += 6;
                                                                } else {
                                                                  r3 = null;
                                                                  if (reportFailures === 0) {
                                                                    matchFailed("\"source\"");
                                                                  }
                                                                }
                                                                if (r3 === null) {
                                                                  if (input.substr(pos, 6) === "select") {
                                                                    r3 = "select";
                                                                    pos += 6;
                                                                  } else {
                                                                    r3 = null;
                                                                    if (reportFailures === 0) {
                                                                      matchFailed("\"select\"");
                                                                    }
                                                                  }
                                                                  if (r3 === null) {
                                                                    if (input.substr(pos, 6) === "script") {
                                                                      r3 = "script";
                                                                      pos += 6;
                                                                    } else {
                                                                      r3 = null;
                                                                      if (reportFailures === 0) {
                                                                        matchFailed("\"script\"");
                                                                      }
                                                                    }
                                                                    if (r3 === null) {
                                                                      if (input.substr(pos, 6) === "output") {
                                                                        r3 = "output";
                                                                        pos += 6;
                                                                      } else {
                                                                        r3 = null;
                                                                        if (reportFailures === 0) {
                                                                          matchFailed("\"output\"");
                                                                        }
                                                                      }
                                                                      if (r3 === null) {
                                                                        if (input.substr(pos, 6) === "option") {
                                                                          r3 = "option";
                                                                          pos += 6;
                                                                        } else {
                                                                          r3 = null;
                                                                          if (reportFailures === 0) {
                                                                            matchFailed("\"option\"");
                                                                          }
                                                                        }
                                                                        if (r3 === null) {
                                                                          if (input.substr(pos, 6) === "object") {
                                                                            r3 = "object";
                                                                            pos += 6;
                                                                          } else {
                                                                            r3 = null;
                                                                            if (reportFailures === 0) {
                                                                              matchFailed("\"object\"");
                                                                            }
                                                                          }
                                                                          if (r3 === null) {
                                                                            if (input.substr(pos, 6) === "legend") {
                                                                              r3 = "legend";
                                                                              pos += 6;
                                                                            } else {
                                                                              r3 = null;
                                                                              if (reportFailures === 0) {
                                                                                matchFailed("\"legend\"");
                                                                              }
                                                                            }
                                                                            if (r3 === null) {
                                                                              if (input.substr(pos, 6) === "keygen") {
                                                                                r3 = "keygen";
                                                                                pos += 6;
                                                                              } else {
                                                                                r3 = null;
                                                                                if (reportFailures === 0) {
                                                                                  matchFailed("\"keygen\"");
                                                                                }
                                                                              }
                                                                              if (r3 === null) {
                                                                                if (input.substr(pos, 6) === "iframe") {
                                                                                  r3 = "iframe";
                                                                                  pos += 6;
                                                                                } else {
                                                                                  r3 = null;
                                                                                  if (reportFailures === 0) {
                                                                                    matchFailed("\"iframe\"");
                                                                                  }
                                                                                }
                                                                                if (r3 === null) {
                                                                                  if (input.substr(pos, 6) === "hgroup") {
                                                                                    r3 = "hgroup";
                                                                                    pos += 6;
                                                                                  } else {
                                                                                    r3 = null;
                                                                                    if (reportFailures === 0) {
                                                                                      matchFailed("\"hgroup\"");
                                                                                    }
                                                                                  }
                                                                                  if (r3 === null) {
                                                                                    if (input.substr(pos, 6) === "header") {
                                                                                      r3 = "header";
                                                                                      pos += 6;
                                                                                    } else {
                                                                                      r3 = null;
                                                                                      if (reportFailures === 0) {
                                                                                        matchFailed("\"header\"");
                                                                                      }
                                                                                    }
                                                                                    if (r3 === null) {
                                                                                      if (input.substr(pos, 6) === "footer") {
                                                                                        r3 = "footer";
                                                                                        pos += 6;
                                                                                      } else {
                                                                                        r3 = null;
                                                                                        if (reportFailures === 0) {
                                                                                          matchFailed("\"footer\"");
                                                                                        }
                                                                                      }
                                                                                      if (r3 === null) {
                                                                                        if (input.substr(pos, 6) === "figure") {
                                                                                          r3 = "figure";
                                                                                          pos += 6;
                                                                                        } else {
                                                                                          r3 = null;
                                                                                          if (reportFailures === 0) {
                                                                                            matchFailed("\"figure\"");
                                                                                          }
                                                                                        }
                                                                                        if (r3 === null) {
                                                                                          if (input.substr(pos, 6) === "center") {
                                                                                            r3 = "center";
                                                                                            pos += 6;
                                                                                          } else {
                                                                                            r3 = null;
                                                                                            if (reportFailures === 0) {
                                                                                              matchFailed("\"center\"");
                                                                                            }
                                                                                          }
                                                                                          if (r3 === null) {
                                                                                            if (input.substr(pos, 6) === "canvas") {
                                                                                              r3 = "canvas";
                                                                                              pos += 6;
                                                                                            } else {
                                                                                              r3 = null;
                                                                                              if (reportFailures === 0) {
                                                                                                matchFailed("\"canvas\"");
                                                                                              }
                                                                                            }
                                                                                            if (r3 === null) {
                                                                                              if (input.substr(pos, 6) === "button") {
                                                                                                r3 = "button";
                                                                                                pos += 6;
                                                                                              } else {
                                                                                                r3 = null;
                                                                                                if (reportFailures === 0) {
                                                                                                  matchFailed("\"button\"");
                                                                                                }
                                                                                              }
                                                                                              if (r3 === null) {
                                                                                                if (input.substr(pos, 6) === "applet") {
                                                                                                  r3 = "applet";
                                                                                                  pos += 6;
                                                                                                } else {
                                                                                                  r3 = null;
                                                                                                  if (reportFailures === 0) {
                                                                                                    matchFailed("\"applet\"");
                                                                                                  }
                                                                                                }
                                                                                                if (r3 === null) {
                                                                                                  if (input.substr(pos, 5) === "video") {
                                                                                                    r3 = "video";
                                                                                                    pos += 5;
                                                                                                  } else {
                                                                                                    r3 = null;
                                                                                                    if (reportFailures === 0) {
                                                                                                      matchFailed("\"video\"");
                                                                                                    }
                                                                                                  }
                                                                                                  if (r3 === null) {
                                                                                                    if (input.substr(pos, 5) === "track") {
                                                                                                      r3 = "track";
                                                                                                      pos += 5;
                                                                                                    } else {
                                                                                                      r3 = null;
                                                                                                      if (reportFailures === 0) {
                                                                                                        matchFailed("\"track\"");
                                                                                                      }
                                                                                                    }
                                                                                                    if (r3 === null) {
                                                                                                      if (input.substr(pos, 5) === "title") {
                                                                                                        r3 = "title";
                                                                                                        pos += 5;
                                                                                                      } else {
                                                                                                        r3 = null;
                                                                                                        if (reportFailures === 0) {
                                                                                                          matchFailed("\"title\"");
                                                                                                        }
                                                                                                      }
                                                                                                      if (r3 === null) {
                                                                                                        if (input.substr(pos, 5) === "thead") {
                                                                                                          r3 = "thead";
                                                                                                          pos += 5;
                                                                                                        } else {
                                                                                                          r3 = null;
                                                                                                          if (reportFailures === 0) {
                                                                                                            matchFailed("\"thead\"");
                                                                                                          }
                                                                                                        }
                                                                                                        if (r3 === null) {
                                                                                                          if (input.substr(pos, 5) === "tfoot") {
                                                                                                            r3 = "tfoot";
                                                                                                            pos += 5;
                                                                                                          } else {
                                                                                                            r3 = null;
                                                                                                            if (reportFailures === 0) {
                                                                                                              matchFailed("\"tfoot\"");
                                                                                                            }
                                                                                                          }
                                                                                                          if (r3 === null) {
                                                                                                            if (input.substr(pos, 5) === "tbody") {
                                                                                                              r3 = "tbody";
                                                                                                              pos += 5;
                                                                                                            } else {
                                                                                                              r3 = null;
                                                                                                              if (reportFailures === 0) {
                                                                                                                matchFailed("\"tbody\"");
                                                                                                              }
                                                                                                            }
                                                                                                            if (r3 === null) {
                                                                                                              if (input.substr(pos, 5) === "table") {
                                                                                                                r3 = "table";
                                                                                                                pos += 5;
                                                                                                              } else {
                                                                                                                r3 = null;
                                                                                                                if (reportFailures === 0) {
                                                                                                                  matchFailed("\"table\"");
                                                                                                                }
                                                                                                              }
                                                                                                              if (r3 === null) {
                                                                                                                if (input.substr(pos, 5) === "style") {
                                                                                                                  r3 = "style";
                                                                                                                  pos += 5;
                                                                                                                } else {
                                                                                                                  r3 = null;
                                                                                                                  if (reportFailures === 0) {
                                                                                                                    matchFailed("\"style\"");
                                                                                                                  }
                                                                                                                }
                                                                                                                if (r3 === null) {
                                                                                                                  if (input.substr(pos, 5) === "small") {
                                                                                                                    r3 = "small";
                                                                                                                    pos += 5;
                                                                                                                  } else {
                                                                                                                    r3 = null;
                                                                                                                    if (reportFailures === 0) {
                                                                                                                      matchFailed("\"small\"");
                                                                                                                    }
                                                                                                                  }
                                                                                                                  if (r3 === null) {
                                                                                                                    if (input.substr(pos, 5) === "param") {
                                                                                                                      r3 = "param";
                                                                                                                      pos += 5;
                                                                                                                    } else {
                                                                                                                      r3 = null;
                                                                                                                      if (reportFailures === 0) {
                                                                                                                        matchFailed("\"param\"");
                                                                                                                      }
                                                                                                                    }
                                                                                                                    if (r3 === null) {
                                                                                                                      if (input.substr(pos, 5) === "meter") {
                                                                                                                        r3 = "meter";
                                                                                                                        pos += 5;
                                                                                                                      } else {
                                                                                                                        r3 = null;
                                                                                                                        if (reportFailures === 0) {
                                                                                                                          matchFailed("\"meter\"");
                                                                                                                        }
                                                                                                                      }
                                                                                                                      if (r3 === null) {
                                                                                                                        if (input.substr(pos, 5) === "label") {
                                                                                                                          r3 = "label";
                                                                                                                          pos += 5;
                                                                                                                        } else {
                                                                                                                          r3 = null;
                                                                                                                          if (reportFailures === 0) {
                                                                                                                            matchFailed("\"label\"");
                                                                                                                          }
                                                                                                                        }
                                                                                                                        if (r3 === null) {
                                                                                                                          if (input.substr(pos, 5) === "input") {
                                                                                                                            r3 = "input";
                                                                                                                            pos += 5;
                                                                                                                          } else {
                                                                                                                            r3 = null;
                                                                                                                            if (reportFailures === 0) {
                                                                                                                              matchFailed("\"input\"");
                                                                                                                            }
                                                                                                                          }
                                                                                                                          if (r3 === null) {
                                                                                                                            if (input.substr(pos, 5) === "frame") {
                                                                                                                              r3 = "frame";
                                                                                                                              pos += 5;
                                                                                                                            } else {
                                                                                                                              r3 = null;
                                                                                                                              if (reportFailures === 0) {
                                                                                                                                matchFailed("\"frame\"");
                                                                                                                              }
                                                                                                                            }
                                                                                                                            if (r3 === null) {
                                                                                                                              if (input.substr(pos, 5) === "embed") {
                                                                                                                                r3 = "embed";
                                                                                                                                pos += 5;
                                                                                                                              } else {
                                                                                                                                r3 = null;
                                                                                                                                if (reportFailures === 0) {
                                                                                                                                  matchFailed("\"embed\"");
                                                                                                                                }
                                                                                                                              }
                                                                                                                              if (r3 === null) {
                                                                                                                                if (input.substr(pos, 5) === "blink") {
                                                                                                                                  r3 = "blink";
                                                                                                                                  pos += 5;
                                                                                                                                } else {
                                                                                                                                  r3 = null;
                                                                                                                                  if (reportFailures === 0) {
                                                                                                                                    matchFailed("\"blink\"");
                                                                                                                                  }
                                                                                                                                }
                                                                                                                                if (r3 === null) {
                                                                                                                                  if (input.substr(pos, 5) === "audio") {
                                                                                                                                    r3 = "audio";
                                                                                                                                    pos += 5;
                                                                                                                                  } else {
                                                                                                                                    r3 = null;
                                                                                                                                    if (reportFailures === 0) {
                                                                                                                                      matchFailed("\"audio\"");
                                                                                                                                    }
                                                                                                                                  }
                                                                                                                                  if (r3 === null) {
                                                                                                                                    if (input.substr(pos, 5) === "aside") {
                                                                                                                                      r3 = "aside";
                                                                                                                                      pos += 5;
                                                                                                                                    } else {
                                                                                                                                      r3 = null;
                                                                                                                                      if (reportFailures === 0) {
                                                                                                                                        matchFailed("\"aside\"");
                                                                                                                                      }
                                                                                                                                    }
                                                                                                                                    if (r3 === null) {
                                                                                                                                      if (input.substr(pos, 4) === "time") {
                                                                                                                                        r3 = "time";
                                                                                                                                        pos += 4;
                                                                                                                                      } else {
                                                                                                                                        r3 = null;
                                                                                                                                        if (reportFailures === 0) {
                                                                                                                                          matchFailed("\"time\"");
                                                                                                                                        }
                                                                                                                                      }
                                                                                                                                      if (r3 === null) {
                                                                                                                                        if (input.substr(pos, 4) === "span") {
                                                                                                                                          r3 = "span";
                                                                                                                                          pos += 4;
                                                                                                                                        } else {
                                                                                                                                          r3 = null;
                                                                                                                                          if (reportFailures === 0) {
                                                                                                                                            matchFailed("\"span\"");
                                                                                                                                          }
                                                                                                                                        }
                                                                                                                                        if (r3 === null) {
                                                                                                                                          if (input.substr(pos, 4) === "samp") {
                                                                                                                                            r3 = "samp";
                                                                                                                                            pos += 4;
                                                                                                                                          } else {
                                                                                                                                            r3 = null;
                                                                                                                                            if (reportFailures === 0) {
                                                                                                                                              matchFailed("\"samp\"");
                                                                                                                                            }
                                                                                                                                          }
                                                                                                                                          if (r3 === null) {
                                                                                                                                            if (input.substr(pos, 4) === "ruby") {
                                                                                                                                              r3 = "ruby";
                                                                                                                                              pos += 4;
                                                                                                                                            } else {
                                                                                                                                              r3 = null;
                                                                                                                                              if (reportFailures === 0) {
                                                                                                                                                matchFailed("\"ruby\"");
                                                                                                                                              }
                                                                                                                                            }
                                                                                                                                            if (r3 === null) {
                                                                                                                                              if (input.substr(pos, 4) === "nobr") {
                                                                                                                                                r3 = "nobr";
                                                                                                                                                pos += 4;
                                                                                                                                              } else {
                                                                                                                                                r3 = null;
                                                                                                                                                if (reportFailures === 0) {
                                                                                                                                                  matchFailed("\"nobr\"");
                                                                                                                                                }
                                                                                                                                              }
                                                                                                                                              if (r3 === null) {
                                                                                                                                                if (input.substr(pos, 4) === "meta") {
                                                                                                                                                  r3 = "meta";
                                                                                                                                                  pos += 4;
                                                                                                                                                } else {
                                                                                                                                                  r3 = null;
                                                                                                                                                  if (reportFailures === 0) {
                                                                                                                                                    matchFailed("\"meta\"");
                                                                                                                                                  }
                                                                                                                                                }
                                                                                                                                                if (r3 === null) {
                                                                                                                                                  if (input.substr(pos, 4) === "menu") {
                                                                                                                                                    r3 = "menu";
                                                                                                                                                    pos += 4;
                                                                                                                                                  } else {
                                                                                                                                                    r3 = null;
                                                                                                                                                    if (reportFailures === 0) {
                                                                                                                                                      matchFailed("\"menu\"");
                                                                                                                                                    }
                                                                                                                                                  }
                                                                                                                                                  if (r3 === null) {
                                                                                                                                                    if (input.substr(pos, 4) === "mark") {
                                                                                                                                                      r3 = "mark";
                                                                                                                                                      pos += 4;
                                                                                                                                                    } else {
                                                                                                                                                      r3 = null;
                                                                                                                                                      if (reportFailures === 0) {
                                                                                                                                                        matchFailed("\"mark\"");
                                                                                                                                                      }
                                                                                                                                                    }
                                                                                                                                                    if (r3 === null) {
                                                                                                                                                      if (input.substr(pos, 4) === "main") {
                                                                                                                                                        r3 = "main";
                                                                                                                                                        pos += 4;
                                                                                                                                                      } else {
                                                                                                                                                        r3 = null;
                                                                                                                                                        if (reportFailures === 0) {
                                                                                                                                                          matchFailed("\"main\"");
                                                                                                                                                        }
                                                                                                                                                      }
                                                                                                                                                      if (r3 === null) {
                                                                                                                                                        if (input.substr(pos, 4) === "link") {
                                                                                                                                                          r3 = "link";
                                                                                                                                                          pos += 4;
                                                                                                                                                        } else {
                                                                                                                                                          r3 = null;
                                                                                                                                                          if (reportFailures === 0) {
                                                                                                                                                            matchFailed("\"link\"");
                                                                                                                                                          }
                                                                                                                                                        }
                                                                                                                                                        if (r3 === null) {
                                                                                                                                                          if (input.substr(pos, 4) === "html") {
                                                                                                                                                            r3 = "html";
                                                                                                                                                            pos += 4;
                                                                                                                                                          } else {
                                                                                                                                                            r3 = null;
                                                                                                                                                            if (reportFailures === 0) {
                                                                                                                                                              matchFailed("\"html\"");
                                                                                                                                                            }
                                                                                                                                                          }
                                                                                                                                                          if (r3 === null) {
                                                                                                                                                            if (input.substr(pos, 4) === "head") {
                                                                                                                                                              r3 = "head";
                                                                                                                                                              pos += 4;
                                                                                                                                                            } else {
                                                                                                                                                              r3 = null;
                                                                                                                                                              if (reportFailures === 0) {
                                                                                                                                                                matchFailed("\"head\"");
                                                                                                                                                              }
                                                                                                                                                            }
                                                                                                                                                            if (r3 === null) {
                                                                                                                                                              if (input.substr(pos, 4) === "form") {
                                                                                                                                                                r3 = "form";
                                                                                                                                                                pos += 4;
                                                                                                                                                              } else {
                                                                                                                                                                r3 = null;
                                                                                                                                                                if (reportFailures === 0) {
                                                                                                                                                                  matchFailed("\"form\"");
                                                                                                                                                                }
                                                                                                                                                              }
                                                                                                                                                              if (r3 === null) {
                                                                                                                                                                if (input.substr(pos, 4) === "font") {
                                                                                                                                                                  r3 = "font";
                                                                                                                                                                  pos += 4;
                                                                                                                                                                } else {
                                                                                                                                                                  r3 = null;
                                                                                                                                                                  if (reportFailures === 0) {
                                                                                                                                                                    matchFailed("\"font\"");
                                                                                                                                                                  }
                                                                                                                                                                }
                                                                                                                                                                if (r3 === null) {
                                                                                                                                                                  if (input.substr(pos, 4) === "data") {
                                                                                                                                                                    r3 = "data";
                                                                                                                                                                    pos += 4;
                                                                                                                                                                  } else {
                                                                                                                                                                    r3 = null;
                                                                                                                                                                    if (reportFailures === 0) {
                                                                                                                                                                      matchFailed("\"data\"");
                                                                                                                                                                    }
                                                                                                                                                                  }
                                                                                                                                                                  if (r3 === null) {
                                                                                                                                                                    if (input.substr(pos, 4) === "code") {
                                                                                                                                                                      r3 = "code";
                                                                                                                                                                      pos += 4;
                                                                                                                                                                    } else {
                                                                                                                                                                      r3 = null;
                                                                                                                                                                      if (reportFailures === 0) {
                                                                                                                                                                        matchFailed("\"code\"");
                                                                                                                                                                      }
                                                                                                                                                                    }
                                                                                                                                                                    if (r3 === null) {
                                                                                                                                                                      if (input.substr(pos, 4) === "cite") {
                                                                                                                                                                        r3 = "cite";
                                                                                                                                                                        pos += 4;
                                                                                                                                                                      } else {
                                                                                                                                                                        r3 = null;
                                                                                                                                                                        if (reportFailures === 0) {
                                                                                                                                                                          matchFailed("\"cite\"");
                                                                                                                                                                        }
                                                                                                                                                                      }
                                                                                                                                                                      if (r3 === null) {
                                                                                                                                                                        if (input.substr(pos, 4) === "body") {
                                                                                                                                                                          r3 = "body";
                                                                                                                                                                          pos += 4;
                                                                                                                                                                        } else {
                                                                                                                                                                          r3 = null;
                                                                                                                                                                          if (reportFailures === 0) {
                                                                                                                                                                            matchFailed("\"body\"");
                                                                                                                                                                          }
                                                                                                                                                                        }
                                                                                                                                                                        if (r3 === null) {
                                                                                                                                                                          if (input.substr(pos, 4) === "base") {
                                                                                                                                                                            r3 = "base";
                                                                                                                                                                            pos += 4;
                                                                                                                                                                          } else {
                                                                                                                                                                            r3 = null;
                                                                                                                                                                            if (reportFailures === 0) {
                                                                                                                                                                              matchFailed("\"base\"");
                                                                                                                                                                            }
                                                                                                                                                                          }
                                                                                                                                                                          if (r3 === null) {
                                                                                                                                                                            if (input.substr(pos, 4) === "area") {
                                                                                                                                                                              r3 = "area";
                                                                                                                                                                              pos += 4;
                                                                                                                                                                            } else {
                                                                                                                                                                              r3 = null;
                                                                                                                                                                              if (reportFailures === 0) {
                                                                                                                                                                                matchFailed("\"area\"");
                                                                                                                                                                              }
                                                                                                                                                                            }
                                                                                                                                                                            if (r3 === null) {
                                                                                                                                                                              if (input.substr(pos, 4) === "abbr") {
                                                                                                                                                                                r3 = "abbr";
                                                                                                                                                                                pos += 4;
                                                                                                                                                                              } else {
                                                                                                                                                                                r3 = null;
                                                                                                                                                                                if (reportFailures === 0) {
                                                                                                                                                                                  matchFailed("\"abbr\"");
                                                                                                                                                                                }
                                                                                                                                                                              }
                                                                                                                                                                              if (r3 === null) {
                                                                                                                                                                                if (input.substr(pos, 3) === "xmp") {
                                                                                                                                                                                  r3 = "xmp";
                                                                                                                                                                                  pos += 3;
                                                                                                                                                                                } else {
                                                                                                                                                                                  r3 = null;
                                                                                                                                                                                  if (reportFailures === 0) {
                                                                                                                                                                                    matchFailed("\"xmp\"");
                                                                                                                                                                                  }
                                                                                                                                                                                }
                                                                                                                                                                                if (r3 === null) {
                                                                                                                                                                                  if (input.substr(pos, 3) === "wbr") {
                                                                                                                                                                                    r3 = "wbr";
                                                                                                                                                                                    pos += 3;
                                                                                                                                                                                  } else {
                                                                                                                                                                                    r3 = null;
                                                                                                                                                                                    if (reportFailures === 0) {
                                                                                                                                                                                      matchFailed("\"wbr\"");
                                                                                                                                                                                    }
                                                                                                                                                                                  }
                                                                                                                                                                                  if (r3 === null) {
                                                                                                                                                                                    if (input.substr(pos, 3) === "var") {
                                                                                                                                                                                      r3 = "var";
                                                                                                                                                                                      pos += 3;
                                                                                                                                                                                    } else {
                                                                                                                                                                                      r3 = null;
                                                                                                                                                                                      if (reportFailures === 0) {
                                                                                                                                                                                        matchFailed("\"var\"");
                                                                                                                                                                                      }
                                                                                                                                                                                    }
                                                                                                                                                                                    if (r3 === null) {
                                                                                                                                                                                      if (input.substr(pos, 3) === "sup") {
                                                                                                                                                                                        r3 = "sup";
                                                                                                                                                                                        pos += 3;
                                                                                                                                                                                      } else {
                                                                                                                                                                                        r3 = null;
                                                                                                                                                                                        if (reportFailures === 0) {
                                                                                                                                                                                          matchFailed("\"sup\"");
                                                                                                                                                                                        }
                                                                                                                                                                                      }
                                                                                                                                                                                      if (r3 === null) {
                                                                                                                                                                                        if (input.substr(pos, 3) === "sub") {
                                                                                                                                                                                          r3 = "sub";
                                                                                                                                                                                          pos += 3;
                                                                                                                                                                                        } else {
                                                                                                                                                                                          r3 = null;
                                                                                                                                                                                          if (reportFailures === 0) {
                                                                                                                                                                                            matchFailed("\"sub\"");
                                                                                                                                                                                          }
                                                                                                                                                                                        }
                                                                                                                                                                                        if (r3 === null) {
                                                                                                                                                                                          if (input.substr(pos, 3) === "pre") {
                                                                                                                                                                                            r3 = "pre";
                                                                                                                                                                                            pos += 3;
                                                                                                                                                                                          } else {
                                                                                                                                                                                            r3 = null;
                                                                                                                                                                                            if (reportFailures === 0) {
                                                                                                                                                                                              matchFailed("\"pre\"");
                                                                                                                                                                                            }
                                                                                                                                                                                          }
                                                                                                                                                                                          if (r3 === null) {
                                                                                                                                                                                            if (input.substr(pos, 3) === "nav") {
                                                                                                                                                                                              r3 = "nav";
                                                                                                                                                                                              pos += 3;
                                                                                                                                                                                            } else {
                                                                                                                                                                                              r3 = null;
                                                                                                                                                                                              if (reportFailures === 0) {
                                                                                                                                                                                                matchFailed("\"nav\"");
                                                                                                                                                                                              }
                                                                                                                                                                                            }
                                                                                                                                                                                            if (r3 === null) {
                                                                                                                                                                                              if (input.substr(pos, 3) === "map") {
                                                                                                                                                                                                r3 = "map";
                                                                                                                                                                                                pos += 3;
                                                                                                                                                                                              } else {
                                                                                                                                                                                                r3 = null;
                                                                                                                                                                                                if (reportFailures === 0) {
                                                                                                                                                                                                  matchFailed("\"map\"");
                                                                                                                                                                                                }
                                                                                                                                                                                              }
                                                                                                                                                                                              if (r3 === null) {
                                                                                                                                                                                                if (input.substr(pos, 3) === "kbd") {
                                                                                                                                                                                                  r3 = "kbd";
                                                                                                                                                                                                  pos += 3;
                                                                                                                                                                                                } else {
                                                                                                                                                                                                  r3 = null;
                                                                                                                                                                                                  if (reportFailures === 0) {
                                                                                                                                                                                                    matchFailed("\"kbd\"");
                                                                                                                                                                                                  }
                                                                                                                                                                                                }
                                                                                                                                                                                                if (r3 === null) {
                                                                                                                                                                                                  if (input.substr(pos, 3) === "ins") {
                                                                                                                                                                                                    r3 = "ins";
                                                                                                                                                                                                    pos += 3;
                                                                                                                                                                                                  } else {
                                                                                                                                                                                                    r3 = null;
                                                                                                                                                                                                    if (reportFailures === 0) {
                                                                                                                                                                                                      matchFailed("\"ins\"");
                                                                                                                                                                                                    }
                                                                                                                                                                                                  }
                                                                                                                                                                                                  if (r3 === null) {
                                                                                                                                                                                                    if (input.substr(pos, 3) === "img") {
                                                                                                                                                                                                      r3 = "img";
                                                                                                                                                                                                      pos += 3;
                                                                                                                                                                                                    } else {
                                                                                                                                                                                                      r3 = null;
                                                                                                                                                                                                      if (reportFailures === 0) {
                                                                                                                                                                                                        matchFailed("\"img\"");
                                                                                                                                                                                                      }
                                                                                                                                                                                                    }
                                                                                                                                                                                                    if (r3 === null) {
                                                                                                                                                                                                      if (input.substr(pos, 3) === "div") {
                                                                                                                                                                                                        r3 = "div";
                                                                                                                                                                                                        pos += 3;
                                                                                                                                                                                                      } else {
                                                                                                                                                                                                        r3 = null;
                                                                                                                                                                                                        if (reportFailures === 0) {
                                                                                                                                                                                                          matchFailed("\"div\"");
                                                                                                                                                                                                        }
                                                                                                                                                                                                      }
                                                                                                                                                                                                      if (r3 === null) {
                                                                                                                                                                                                        if (input.substr(pos, 3) === "dir") {
                                                                                                                                                                                                          r3 = "dir";
                                                                                                                                                                                                          pos += 3;
                                                                                                                                                                                                        } else {
                                                                                                                                                                                                          r3 = null;
                                                                                                                                                                                                          if (reportFailures === 0) {
                                                                                                                                                                                                            matchFailed("\"dir\"");
                                                                                                                                                                                                          }
                                                                                                                                                                                                        }
                                                                                                                                                                                                        if (r3 === null) {
                                                                                                                                                                                                          if (input.substr(pos, 3) === "dfn") {
                                                                                                                                                                                                            r3 = "dfn";
                                                                                                                                                                                                            pos += 3;
                                                                                                                                                                                                          } else {
                                                                                                                                                                                                            r3 = null;
                                                                                                                                                                                                            if (reportFailures === 0) {
                                                                                                                                                                                                              matchFailed("\"dfn\"");
                                                                                                                                                                                                            }
                                                                                                                                                                                                          }
                                                                                                                                                                                                          if (r3 === null) {
                                                                                                                                                                                                            if (input.substr(pos, 3) === "del") {
                                                                                                                                                                                                              r3 = "del";
                                                                                                                                                                                                              pos += 3;
                                                                                                                                                                                                            } else {
                                                                                                                                                                                                              r3 = null;
                                                                                                                                                                                                              if (reportFailures === 0) {
                                                                                                                                                                                                                matchFailed("\"del\"");
                                                                                                                                                                                                              }
                                                                                                                                                                                                            }
                                                                                                                                                                                                            if (r3 === null) {
                                                                                                                                                                                                              if (input.substr(pos, 3) === "col") {
                                                                                                                                                                                                                r3 = "col";
                                                                                                                                                                                                                pos += 3;
                                                                                                                                                                                                              } else {
                                                                                                                                                                                                                r3 = null;
                                                                                                                                                                                                                if (reportFailures === 0) {
                                                                                                                                                                                                                  matchFailed("\"col\"");
                                                                                                                                                                                                                }
                                                                                                                                                                                                              }
                                                                                                                                                                                                              if (r3 === null) {
                                                                                                                                                                                                                if (input.substr(pos, 3) === "big") {
                                                                                                                                                                                                                  r3 = "big";
                                                                                                                                                                                                                  pos += 3;
                                                                                                                                                                                                                } else {
                                                                                                                                                                                                                  r3 = null;
                                                                                                                                                                                                                  if (reportFailures === 0) {
                                                                                                                                                                                                                    matchFailed("\"big\"");
                                                                                                                                                                                                                  }
                                                                                                                                                                                                                }
                                                                                                                                                                                                                if (r3 === null) {
                                                                                                                                                                                                                  if (input.substr(pos, 3) === "bdo") {
                                                                                                                                                                                                                    r3 = "bdo";
                                                                                                                                                                                                                    pos += 3;
                                                                                                                                                                                                                  } else {
                                                                                                                                                                                                                    r3 = null;
                                                                                                                                                                                                                    if (reportFailures === 0) {
                                                                                                                                                                                                                      matchFailed("\"bdo\"");
                                                                                                                                                                                                                    }
                                                                                                                                                                                                                  }
                                                                                                                                                                                                                  if (r3 === null) {
                                                                                                                                                                                                                    if (input.substr(pos, 3) === "bdi") {
                                                                                                                                                                                                                      r3 = "bdi";
                                                                                                                                                                                                                      pos += 3;
                                                                                                                                                                                                                    } else {
                                                                                                                                                                                                                      r3 = null;
                                                                                                                                                                                                                      if (reportFailures === 0) {
                                                                                                                                                                                                                        matchFailed("\"bdi\"");
                                                                                                                                                                                                                      }
                                                                                                                                                                                                                    }
                                                                                                                                                                                                                    if (r3 === null) {
                                                                                                                                                                                                                      if (input.substr(pos, 2) === "ul") {
                                                                                                                                                                                                                        r3 = "ul";
                                                                                                                                                                                                                        pos += 2;
                                                                                                                                                                                                                      } else {
                                                                                                                                                                                                                        r3 = null;
                                                                                                                                                                                                                        if (reportFailures === 0) {
                                                                                                                                                                                                                          matchFailed("\"ul\"");
                                                                                                                                                                                                                        }
                                                                                                                                                                                                                      }
                                                                                                                                                                                                                      if (r3 === null) {
                                                                                                                                                                                                                        if (input.substr(pos, 2) === "tt") {
                                                                                                                                                                                                                          r3 = "tt";
                                                                                                                                                                                                                          pos += 2;
                                                                                                                                                                                                                        } else {
                                                                                                                                                                                                                          r3 = null;
                                                                                                                                                                                                                          if (reportFailures === 0) {
                                                                                                                                                                                                                            matchFailed("\"tt\"");
                                                                                                                                                                                                                          }
                                                                                                                                                                                                                        }
                                                                                                                                                                                                                        if (r3 === null) {
                                                                                                                                                                                                                          if (input.substr(pos, 2) === "tr") {
                                                                                                                                                                                                                            r3 = "tr";
                                                                                                                                                                                                                            pos += 2;
                                                                                                                                                                                                                          } else {
                                                                                                                                                                                                                            r3 = null;
                                                                                                                                                                                                                            if (reportFailures === 0) {
                                                                                                                                                                                                                              matchFailed("\"tr\"");
                                                                                                                                                                                                                            }
                                                                                                                                                                                                                          }
                                                                                                                                                                                                                          if (r3 === null) {
                                                                                                                                                                                                                            if (input.substr(pos, 2) === "th") {
                                                                                                                                                                                                                              r3 = "th";
                                                                                                                                                                                                                              pos += 2;
                                                                                                                                                                                                                            } else {
                                                                                                                                                                                                                              r3 = null;
                                                                                                                                                                                                                              if (reportFailures === 0) {
                                                                                                                                                                                                                                matchFailed("\"th\"");
                                                                                                                                                                                                                              }
                                                                                                                                                                                                                            }
                                                                                                                                                                                                                            if (r3 === null) {
                                                                                                                                                                                                                              if (input.substr(pos, 2) === "td") {
                                                                                                                                                                                                                                r3 = "td";
                                                                                                                                                                                                                                pos += 2;
                                                                                                                                                                                                                              } else {
                                                                                                                                                                                                                                r3 = null;
                                                                                                                                                                                                                                if (reportFailures === 0) {
                                                                                                                                                                                                                                  matchFailed("\"td\"");
                                                                                                                                                                                                                                }
                                                                                                                                                                                                                              }
                                                                                                                                                                                                                              if (r3 === null) {
                                                                                                                                                                                                                                if (input.substr(pos, 2) === "rt") {
                                                                                                                                                                                                                                  r3 = "rt";
                                                                                                                                                                                                                                  pos += 2;
                                                                                                                                                                                                                                } else {
                                                                                                                                                                                                                                  r3 = null;
                                                                                                                                                                                                                                  if (reportFailures === 0) {
                                                                                                                                                                                                                                    matchFailed("\"rt\"");
                                                                                                                                                                                                                                  }
                                                                                                                                                                                                                                }
                                                                                                                                                                                                                                if (r3 === null) {
                                                                                                                                                                                                                                  if (input.substr(pos, 2) === "rp") {
                                                                                                                                                                                                                                    r3 = "rp";
                                                                                                                                                                                                                                    pos += 2;
                                                                                                                                                                                                                                  } else {
                                                                                                                                                                                                                                    r3 = null;
                                                                                                                                                                                                                                    if (reportFailures === 0) {
                                                                                                                                                                                                                                      matchFailed("\"rp\"");
                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                  }
                                                                                                                                                                                                                                  if (r3 === null) {
                                                                                                                                                                                                                                    if (input.substr(pos, 2) === "ol") {
                                                                                                                                                                                                                                      r3 = "ol";
                                                                                                                                                                                                                                      pos += 2;
                                                                                                                                                                                                                                    } else {
                                                                                                                                                                                                                                      r3 = null;
                                                                                                                                                                                                                                      if (reportFailures === 0) {
                                                                                                                                                                                                                                        matchFailed("\"ol\"");
                                                                                                                                                                                                                                      }
                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                    if (r3 === null) {
                                                                                                                                                                                                                                      if (input.substr(pos, 2) === "li") {
                                                                                                                                                                                                                                        r3 = "li";
                                                                                                                                                                                                                                        pos += 2;
                                                                                                                                                                                                                                      } else {
                                                                                                                                                                                                                                        r3 = null;
                                                                                                                                                                                                                                        if (reportFailures === 0) {
                                                                                                                                                                                                                                          matchFailed("\"li\"");
                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                      }
                                                                                                                                                                                                                                      if (r3 === null) {
                                                                                                                                                                                                                                        if (input.substr(pos, 2) === "hr") {
                                                                                                                                                                                                                                          r3 = "hr";
                                                                                                                                                                                                                                          pos += 2;
                                                                                                                                                                                                                                        } else {
                                                                                                                                                                                                                                          r3 = null;
                                                                                                                                                                                                                                          if (reportFailures === 0) {
                                                                                                                                                                                                                                            matchFailed("\"hr\"");
                                                                                                                                                                                                                                          }
                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                        if (r3 === null) {
                                                                                                                                                                                                                                          if (input.substr(pos, 2) === "h6") {
                                                                                                                                                                                                                                            r3 = "h6";
                                                                                                                                                                                                                                            pos += 2;
                                                                                                                                                                                                                                          } else {
                                                                                                                                                                                                                                            r3 = null;
                                                                                                                                                                                                                                            if (reportFailures === 0) {
                                                                                                                                                                                                                                              matchFailed("\"h6\"");
                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                          }
                                                                                                                                                                                                                                          if (r3 === null) {
                                                                                                                                                                                                                                            if (input.substr(pos, 2) === "h5") {
                                                                                                                                                                                                                                              r3 = "h5";
                                                                                                                                                                                                                                              pos += 2;
                                                                                                                                                                                                                                            } else {
                                                                                                                                                                                                                                              r3 = null;
                                                                                                                                                                                                                                              if (reportFailures === 0) {
                                                                                                                                                                                                                                                matchFailed("\"h5\"");
                                                                                                                                                                                                                                              }
                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                            if (r3 === null) {
                                                                                                                                                                                                                                              if (input.substr(pos, 2) === "h4") {
                                                                                                                                                                                                                                                r3 = "h4";
                                                                                                                                                                                                                                                pos += 2;
                                                                                                                                                                                                                                              } else {
                                                                                                                                                                                                                                                r3 = null;
                                                                                                                                                                                                                                                if (reportFailures === 0) {
                                                                                                                                                                                                                                                  matchFailed("\"h4\"");
                                                                                                                                                                                                                                                }
                                                                                                                                                                                                                                              }
                                                                                                                                                                                                                                              if (r3 === null) {
                                                                                                                                                                                                                                                if (input.substr(pos, 2) === "h3") {
                                                                                                                                                                                                                                                  r3 = "h3";
                                                                                                                                                                                                                                                  pos += 2;
                                                                                                                                                                                                                                                } else {
                                                                                                                                                                                                                                                  r3 = null;
                                                                                                                                                                                                                                                  if (reportFailures === 0) {
                                                                                                                                                                                                                                                    matchFailed("\"h3\"");
                                                                                                                                                                                                                                                  }
                                                                                                                                                                                                                                                }
                                                                                                                                                                                                                                                if (r3 === null) {
                                                                                                                                                                                                                                                  if (input.substr(pos, 2) === "h2") {
                                                                                                                                                                                                                                                    r3 = "h2";
                                                                                                                                                                                                                                                    pos += 2;
                                                                                                                                                                                                                                                  } else {
                                                                                                                                                                                                                                                    r3 = null;
                                                                                                                                                                                                                                                    if (reportFailures === 0) {
                                                                                                                                                                                                                                                      matchFailed("\"h2\"");
                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                  }
                                                                                                                                                                                                                                                  if (r3 === null) {
                                                                                                                                                                                                                                                    if (input.substr(pos, 2) === "h1") {
                                                                                                                                                                                                                                                      r3 = "h1";
                                                                                                                                                                                                                                                      pos += 2;
                                                                                                                                                                                                                                                    } else {
                                                                                                                                                                                                                                                      r3 = null;
                                                                                                                                                                                                                                                      if (reportFailures === 0) {
                                                                                                                                                                                                                                                        matchFailed("\"h1\"");
                                                                                                                                                                                                                                                      }
                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                    if (r3 === null) {
                                                                                                                                                                                                                                                      if (input.substr(pos, 2) === "em") {
                                                                                                                                                                                                                                                        r3 = "em";
                                                                                                                                                                                                                                                        pos += 2;
                                                                                                                                                                                                                                                      } else {
                                                                                                                                                                                                                                                        r3 = null;
                                                                                                                                                                                                                                                        if (reportFailures === 0) {
                                                                                                                                                                                                                                                          matchFailed("\"em\"");
                                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                                      }
                                                                                                                                                                                                                                                      if (r3 === null) {
                                                                                                                                                                                                                                                        if (input.substr(pos, 2) === "dt") {
                                                                                                                                                                                                                                                          r3 = "dt";
                                                                                                                                                                                                                                                          pos += 2;
                                                                                                                                                                                                                                                        } else {
                                                                                                                                                                                                                                                          r3 = null;
                                                                                                                                                                                                                                                          if (reportFailures === 0) {
                                                                                                                                                                                                                                                            matchFailed("\"dt\"");
                                                                                                                                                                                                                                                          }
                                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                                        if (r3 === null) {
                                                                                                                                                                                                                                                          if (input.substr(pos, 2) === "dl") {
                                                                                                                                                                                                                                                            r3 = "dl";
                                                                                                                                                                                                                                                            pos += 2;
                                                                                                                                                                                                                                                          } else {
                                                                                                                                                                                                                                                            r3 = null;
                                                                                                                                                                                                                                                            if (reportFailures === 0) {
                                                                                                                                                                                                                                                              matchFailed("\"dl\"");
                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                          }
                                                                                                                                                                                                                                                          if (r3 === null) {
                                                                                                                                                                                                                                                            if (input.substr(pos, 2) === "dd") {
                                                                                                                                                                                                                                                              r3 = "dd";
                                                                                                                                                                                                                                                              pos += 2;
                                                                                                                                                                                                                                                            } else {
                                                                                                                                                                                                                                                              r3 = null;
                                                                                                                                                                                                                                                              if (reportFailures === 0) {
                                                                                                                                                                                                                                                                matchFailed("\"dd\"");
                                                                                                                                                                                                                                                              }
                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                            if (r3 === null) {
                                                                                                                                                                                                                                                              if (input.substr(pos, 2) === "br") {
                                                                                                                                                                                                                                                                r3 = "br";
                                                                                                                                                                                                                                                                pos += 2;
                                                                                                                                                                                                                                                              } else {
                                                                                                                                                                                                                                                                r3 = null;
                                                                                                                                                                                                                                                                if (reportFailures === 0) {
                                                                                                                                                                                                                                                                  matchFailed("\"br\"");
                                                                                                                                                                                                                                                                }
                                                                                                                                                                                                                                                              }
                                                                                                                                                                                                                                                              if (r3 === null) {
                                                                                                                                                                                                                                                                if (input.charCodeAt(pos) === 117) {
                                                                                                                                                                                                                                                                  r3 = "u";
                                                                                                                                                                                                                                                                  pos++;
                                                                                                                                                                                                                                                                } else {
                                                                                                                                                                                                                                                                  r3 = null;
                                                                                                                                                                                                                                                                  if (reportFailures === 0) {
                                                                                                                                                                                                                                                                    matchFailed("\"u\"");
                                                                                                                                                                                                                                                                  }
                                                                                                                                                                                                                                                                }
                                                                                                                                                                                                                                                                if (r3 === null) {
                                                                                                                                                                                                                                                                  if (input.charCodeAt(pos) === 115) {
                                                                                                                                                                                                                                                                    r3 = "s";
                                                                                                                                                                                                                                                                    pos++;
                                                                                                                                                                                                                                                                  } else {
                                                                                                                                                                                                                                                                    r3 = null;
                                                                                                                                                                                                                                                                    if (reportFailures === 0) {
                                                                                                                                                                                                                                                                      matchFailed("\"s\"");
                                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                                  }
                                                                                                                                                                                                                                                                  if (r3 === null) {
                                                                                                                                                                                                                                                                    if (input.charCodeAt(pos) === 113) {
                                                                                                                                                                                                                                                                      r3 = "q";
                                                                                                                                                                                                                                                                      pos++;
                                                                                                                                                                                                                                                                    } else {
                                                                                                                                                                                                                                                                      r3 = null;
                                                                                                                                                                                                                                                                      if (reportFailures === 0) {
                                                                                                                                                                                                                                                                        matchFailed("\"q\"");
                                                                                                                                                                                                                                                                      }
                                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                                    if (r3 === null) {
                                                                                                                                                                                                                                                                      if (input.charCodeAt(pos) === 112) {
                                                                                                                                                                                                                                                                        r3 = "p";
                                                                                                                                                                                                                                                                        pos++;
                                                                                                                                                                                                                                                                      } else {
                                                                                                                                                                                                                                                                        r3 = null;
                                                                                                                                                                                                                                                                        if (reportFailures === 0) {
                                                                                                                                                                                                                                                                          matchFailed("\"p\"");
                                                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                                                      }
                                                                                                                                                                                                                                                                      if (r3 === null) {
                                                                                                                                                                                                                                                                        if (input.charCodeAt(pos) === 105) {
                                                                                                                                                                                                                                                                          r3 = "i";
                                                                                                                                                                                                                                                                          pos++;
                                                                                                                                                                                                                                                                        } else {
                                                                                                                                                                                                                                                                          r3 = null;
                                                                                                                                                                                                                                                                          if (reportFailures === 0) {
                                                                                                                                                                                                                                                                            matchFailed("\"i\"");
                                                                                                                                                                                                                                                                          }
                                                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                                                        if (r3 === null) {
                                                                                                                                                                                                                                                                          if (input.charCodeAt(pos) === 98) {
                                                                                                                                                                                                                                                                            r3 = "b";
                                                                                                                                                                                                                                                                            pos++;
                                                                                                                                                                                                                                                                          } else {
                                                                                                                                                                                                                                                                            r3 = null;
                                                                                                                                                                                                                                                                            if (reportFailures === 0) {
                                                                                                                                                                                                                                                                              matchFailed("\"b\"");
                                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                                          }
                                                                                                                                                                                                                                                                          if (r3 === null) {
                                                                                                                                                                                                                                                                            if (input.charCodeAt(pos) === 97) {
                                                                                                                                                                                                                                                                              r3 = "a";
                                                                                                                                                                                                                                                                              pos++;
                                                                                                                                                                                                                                                                            } else {
                                                                                                                                                                                                                                                                              r3 = null;
                                                                                                                                                                                                                                                                              if (reportFailures === 0) {
                                                                                                                                                                                                                                                                                matchFailed("\"a\"");
                                                                                                                                                                                                                                                                              }
                                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                                          }
                                                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                                                      }
                                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                                  }
                                                                                                                                                                                                                                                                }
                                                                                                                                                                                                                                                              }
                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                          }
                                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                                      }
                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                  }
                                                                                                                                                                                                                                                }
                                                                                                                                                                                                                                              }
                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                          }
                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                      }
                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                  }
                                                                                                                                                                                                                                }
                                                                                                                                                                                                                              }
                                                                                                                                                                                                                            }
                                                                                                                                                                                                                          }
                                                                                                                                                                                                                        }
                                                                                                                                                                                                                      }
                                                                                                                                                                                                                    }
                                                                                                                                                                                                                  }
                                                                                                                                                                                                                }
                                                                                                                                                                                                              }
                                                                                                                                                                                                            }
                                                                                                                                                                                                          }
                                                                                                                                                                                                        }
                                                                                                                                                                                                      }
                                                                                                                                                                                                    }
                                                                                                                                                                                                  }
                                                                                                                                                                                                }
                                                                                                                                                                                              }
                                                                                                                                                                                            }
                                                                                                                                                                                          }
                                                                                                                                                                                        }
                                                                                                                                                                                      }
                                                                                                                                                                                    }
                                                                                                                                                                                  }
                                                                                                                                                                                }
                                                                                                                                                                              }
                                                                                                                                                                            }
                                                                                                                                                                          }
                                                                                                                                                                        }
                                                                                                                                                                      }
                                                                                                                                                                    }
                                                                                                                                                                  }
                                                                                                                                                                }
                                                                                                                                                              }
                                                                                                                                                            }
                                                                                                                                                          }
                                                                                                                                                        }
                                                                                                                                                      }
                                                                                                                                                    }
                                                                                                                                                  }
                                                                                                                                                }
                                                                                                                                              }
                                                                                                                                            }
                                                                                                                                          }
                                                                                                                                        }
                                                                                                                                      }
                                                                                                                                    }
                                                                                                                                  }
                                                                                                                                }
                                                                                                                              }
                                                                                                                            }
                                                                                                                          }
                                                                                                                        }
                                                                                                                      }
                                                                                                                    }
                                                                                                                  }
                                                                                                                }
                                                                                                              }
                                                                                                            }
                                                                                                          }
                                                                                                        }
                                                                                                      }
                                                                                                    }
                                                                                                  }
                                                                                                }
                                                                                              }
                                                                                            }
                                                                                          }
                                                                                        }
                                                                                      }
                                                                                    }
                                                                                  }
                                                                                }
                                                                              }
                                                                            }
                                                                          }
                                                                        }
                                                                      }
                                                                    }
                                                                  }
                                                                }
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        }
        if (r3 !== null) {
          r5 = pos;
          reportFailures++;
          r6 = pos;
          reportFailures++;
          r4 = parse_nmchar();
          reportFailures--;
          if (r4 === null) {
            r4 = "";
          } else {
            r4 = null;
            pos = r6;
          }
          reportFailures--;
          if (r4 !== null) {
            r4 = "";
            pos = r5;
          } else {
            r4 = null;
          }
          if (r4 !== null) {
            r0 = [r3, r4];
          } else {
            r0 = null;
            pos = r2;
          }
        } else {
          r0 = null;
          pos = r2;
        }
        if (r0 !== null) {
          reportedPos = r1;
          r0 = (function(t) {return t;})(r3);
        }
        if (r0 === null) {
          pos = r1;
        }
        
        cache[cacheKey] = {
          nextPos: pos,
          result:  r0
        };
        return r0;
      }
      
      
      function cleanupExpected(expected) {
        expected.sort();
        
        var lastExpected = null;
        var cleanExpected = [];
        for (var i = 0; i < expected.length; i++) {
          if (expected[i] !== lastExpected) {
            cleanExpected.push(expected[i]);
            lastExpected = expected[i];
          }
        }
        return cleanExpected;
      }
      
      
      var result = parseFunctions[startRule]();
      
      /*
       * The parser is now in one of the following three states:
       *
       * 1. The parser successfully parsed the whole input.
       *
       *    - |result !== null|
       *    - |pos === input.length|
       *    - |rightmostFailuresExpected| may or may not contain something
       *
       * 2. The parser successfully parsed only a part of the input.
       *
       *    - |result !== null|
       *    - |pos < input.length|
       *    - |rightmostFailuresExpected| may or may not contain something
       *
       * 3. The parser did not successfully parse any part of the input.
       *
       *   - |result === null|
       *   - |pos === 0|
       *   - |rightmostFailuresExpected| contains at least one failure
       *
       * All code following this comment (including called functions) must
       * handle these states.
       */
      if (result === null || pos !== input.length) {
        reportedPos = Math.max(pos, rightmostFailuresPos);
        var found = reportedPos < input.length ? input.charAt(reportedPos) : null;
        var reportedPosDetails = computeReportedPosDetails();
        
        throw new this.SyntaxError(
          cleanupExpected(rightmostFailuresExpected),
          found,
          reportedPos,
          reportedPosDetails.line,
          reportedPosDetails.column
        );
      }
      
      return result;
    }
  };
  
  /* Thrown when a parser encounters a syntax error. */
  
  result.SyntaxError = function(expected, found, offset, line, column) {
    function buildMessage(expected, found) {
      var expectedHumanized, foundHumanized;
      
      switch (expected.length) {
        case 0:
          expectedHumanized = "end of input";
          break;
        case 1:
          expectedHumanized = expected[0];
          break;
        default:
          expectedHumanized = expected.slice(0, expected.length - 1).join(", ")
            + " or "
            + expected[expected.length - 1];
      }
      
      foundHumanized = found ? quote(found) : "end of input";
      
      return "Expected " + expectedHumanized + " but " + foundHumanized + " found.";
    }
    
    this.name = "SyntaxError";
    this.expected = expected;
    this.found = found;
    this.message = buildMessage(expected, found);
    this.offset = offset;
    this.line = line;
    this.column = column;
  };
  
  subclass(result.SyntaxError, Error);
  
  return result;
})();
