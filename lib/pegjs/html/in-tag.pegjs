...
@import "./raw-mustache-attr.pegjs" as rawMustacheAttribute
@import "./normal-attribute.pegjs" as normalAttribute
@import "./tag-string.pegjs" as tagString
@import "../mustache/in-tag.pegjs" as inTagMustache
...
bracketedAttribute
= INDENT* ' '* a:(action / booleanAttribute / boundAttribute / rawMustacheAttribute / normalAttribute) TERM*
{
  if (a.length) {
    return a;
  } else {
...
fullAttribute
  = ' '+ a:(action / booleanAttribute / boundAttributeWithSingleMustache/ boundAttribute / rawMustacheAttribute / normalAttribute)
{
  return a || [];
}
